...
const snake3d = document.getElementById("snake3d");

const renderer = new THREE.WebGLRenderer({ alpha: true, antialias: true });
//renderer.setPixelRatio(1);
renderer.setSize(snake3d.clientWidth, snake3d.clientWidth);
renderer.shadowMap.enabled = true;
renderer.shadowMap.type = THREE.PCFSoftShadowMap;
renderer.domElement.style.outline = "none"
renderer.domElement.style.cursor = "move"
snake3d.prepend(renderer.domElement);
loading.remove();
...
const rectLight = new THREE.RectAreaLight(0xffffff, 1, 1.8, 1.8);
...
const lightHead = new THREE.PointLight(0x44ff44, 0.8);
lightHead.distance = 1.9;
window.li = lightHead;
...
const materialZona = new THREE.MeshStandardMaterial({ color: 0xffFFff, roughness: 0.2, metalness: 0.5, side: 1 });
...
function createBlock(block) {
    ...
    const materialBlock = new THREE.MeshStandardMaterial({ color: 0x44cc44, roughness: 0.2, metalness: 0.5, flatShading: true });
